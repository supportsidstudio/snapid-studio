...
export default function PassportDressPanel({
  ...
  const [isMobileAdjustModalOpen, setIsMobileAdjustModalOpen] = useState<boolean>(false);
  ...
  const handleSelectTemplate = (template: DressTemplate) => {
    if (dressState.templateId === template.id) {
      ...
      setIsMobileAdjustModalOpen(false);
      return;
    }
    ...
    // Smoothly ensure preview canvas is in view without blocking the screen
    if (typeof window !== 'undefined') {
      try {
        const previewEl = document.getElementById('passport-left-preview-panel');
  ...
  return (
    <div className={`p-3.5 sm:p-4 rounded-2xl border subtle-glow-card space-y-3 ${
      ...
      {/* Mobile Quick Re-Open Floating Bottom Bar Button when Suit is Active */}
      {isSuitActive && (
        <div className="md:hidden">
          <button
            type="button"
            onClick={() => {
              setIsMobileAdjustModalOpen(true);
              try {
                document.getElementById('passport-left-preview-panel')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
              } catch {
                // ignore
              }
            }}
            className="w-full py-2.5 px-3 rounded-xl text-xs font-extrabold bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white flex items-center justify-between shadow-md shadow-blue-500/25 cursor-pointer active:scale-98 transition-all border border-blue-400/40"
          >
            <div className="flex items-center gap-2">
              <Sliders className="w-4 h-4 text-cyan-300" />
              <span>{language === 'hi' ? '👔 फ्लोटिंग सूट एडजस्ट टूल खोलें' : '👔 Open Floating Suit Adjust'}</span>
            </div>
      ...
      {/* Fine-Tuning Controls (Visible directly inline on all screens: Mobile + Tablet + Desktop) */}
      {isSuitActive && (
        <div className={`block p-3.5 rounded-xl border space-y-3.5 animate-fadeIn subtle-element-glow ${
      ...
      {/* Dedicated Non-Blocking Floating Mobile Suit Adjustment Bottom Bar */}
      {isMobileAdjustModalOpen && isSuitActive && (
        <div 
          className="fixed bottom-0 inset-x-0 z-50 md:hidden bg-slate-900/98 backdrop-blur-xl border-t-2 border-blue-500/50 shadow-2xl p-3 sm:p-4 rounded-t-3xl max-h-[48vh] overflow-y-auto text-slate-100 select-none animate-in slide-in-from-bottom duration-200"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="w-full max-w-lg mx-auto space-y-3 pb-2">
            {/* Sheet Header */}
            <div className="flex items-center justify-between border-b pb-2.5 border-slate-800">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-extrabold text-xs tracking-tight flex items-center gap-1.5 text-white">
                    <span>{language === 'hi' ? 'सूट एडजस्ट टूल' : 'Suit Adjust Tools'}</span>
                  ...
                  <p className="text-[10px] text-slate-400 font-medium">
                    {language === 'hi' ? 'ऊपर फोटो में लाइव बदलाव देखें ↑' : 'Photo is visible above in real time ↑'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  type="button"
                  onClick={handleReset}
                  className="text-[11px] font-bold text-slate-300 hover:text-white px-2 py-1 rounded-lg border border-slate-700 bg-slate-800 flex items-center gap-1 cursor-pointer active:scale-95"
                  title="Reset Position"
                >
                ...
                <button
                  type="button"
                  onClick={() => setIsMobileAdjustModalOpen(false)}
                  className="p-1 px-2 rounded-lg text-slate-400 hover:text-white bg-slate-800 border border-slate-700 cursor-pointer font-bold text-xs"
                  title="Close / Done"
                >
            ...
            <div className="p-2.5 rounded-xl bg-slate-950/70 border border-slate-800 flex items-center justify-between gap-2">
              <div className="flex flex-col">
                <span className="text-xs font-bold text-slate-200 flex items-center gap-1">
                  ...
                  <span>{language === 'hi' ? 'पोजीशन (Nudge)' : 'Position'}</span>
                </span>
              ...
              <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-lg border border-slate-700/80">
                <button
                  type="button"
                  onClick={() => handleNudge(-1, 0)}
                  className="w-7 h-7 rounded-md bg-slate-800 active:bg-blue-600 text-white flex items-center justify-center font-bold active:scale-95"
                  title="Move Left"
                >
                  <ChevronLeft className="w-3.5 h-3.5" />
                </button>
            ...
            <div className="p-2.5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-1">
              <div className="flex items-center justify-between text-xs">
                <span className="font-bold text-slate-200 text-[11px]">
                  {language === 'hi' ? 'कंधे की चौड़ाई' : 'Shoulder Width'}
                </span>
                <span className="font-mono font-extrabold text-blue-400 bg-blue-500/10 px-1.5 py-0.5 rounded border border-blue-500/20 text-[10px]">
                  {dressState.scaleX.toFixed(2)}x
                </span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => onDressStateChange({ ...dressState, scaleX: Math.max(0.8, parseFloat((dressState.scaleX - 0.02).toFixed(2))) })}
                  className="w-7 h-7 rounded-md bg-slate-800 active:bg-blue-600 text-white flex items-center justify-center font-mono font-bold text-xs"
                >
                  −
                </button>
                <input
                  ...
                  value={dressState.scaleX}
                  onChange={(e) => onDressStateChange({ ...dressState, scaleX: parseFloat(e.target.value) })}
                  className="flex-1 h-1.5 bg-slate-800 rounded-lg appearance-none accent-blue-500 cursor-pointer"
                />
                <button
                  type="button"
                  onClick={() => onDressStateChange({ ...dressState, scaleX: Math.min(1.3, parseFloat((dressState.scaleX + 0.02).toFixed(2))) })}
                  className="w-7 h-7 rounded-md bg-slate-800 active:bg-blue-600 text-white flex items-center justify-center font-mono font-bold text-xs"
                >
                  +
                </button>
            ...
            <div className="p-2.5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-1">
              <div className="flex items-center justify-between text-xs">
                <span className="font-bold text-slate-200 text-[11px]">
                  {language === 'hi' ? 'सूट साइज' : 'Suit Scale'}
                </span>
                <span className="font-mono font-extrabold text-cyan-400 bg-cyan-500/10 px-1.5 py-0.5 rounded border border-cyan-500/20 text-[10px]">
                  {dressState.scale.toFixed(2)}x
                </span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => onDressStateChange({ ...dressState, scale: Math.max(0.7, parseFloat((dressState.scale - 0.02).toFixed(2))) })}
                  className="w-7 h-7 rounded-md bg-slate-800 active:bg-cyan-600 text-white flex items-center justify-center font-mono font-bold text-xs"
                >
                  −
                </button>
                <input
                  ...
                  value={dressState.scale}
                  onChange={(e) => onDressStateChange({ ...dressState, scale: parseFloat(e.target.value) })}
                  className="flex-1 h-1.5 bg-slate-800 rounded-lg appearance-none accent-cyan-500 cursor-pointer"
                />
                <button
                  type="button"
                  onClick={() => onDressStateChange({ ...dressState, scale: Math.min(1.8, parseFloat((dressState.scale + 0.02).toFixed(2))) })}
                  className="w-7 h-7 rounded-md bg-slate-800 active:bg-cyan-600 text-white flex items-center justify-center font-mono font-bold text-xs"
                >
                  +
                </button>
            ...
            <div className="p-2.5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-1">
              <div className="flex items-center justify-between text-xs">
                <span className="font-bold text-slate-200 text-[11px]">
                  {language === 'hi' ? 'झुकाव (Tilt)' : 'Rotation Tilt'}
                </span>
                <span className="font-mono font-extrabold text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20 text-[10px]">
                  {dressState.rotation > 0 ? `+${dressState.rotation}°` : `${dressState.rotation}°`}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => onDressStateChange({ ...dressState, rotation: Math.max(-15, dressState.rotation - 1) })}
                  className="w-7 h-7 rounded-md bg-slate-800 active:bg-amber-600 text-white flex items-center justify-center font-mono font-bold text-xs"
                >
                  −
                </button>
                <input
                  ...
                  value={dressState.rotation}
                  onChange={(e) => onDressStateChange({ ...dressState, rotation: parseInt(e.target.value) })}
                  className="flex-1 h-1.5 bg-slate-800 rounded-lg appearance-none accent-amber-500 cursor-pointer"
                />
                <button
                  type="button"
                  onClick={() => onDressStateChange({ ...dressState, rotation: Math.min(15, dressState.rotation + 1) })}
                  className="w-7 h-7 rounded-md bg-slate-800 active:bg-amber-600 text-white flex items-center justify-center font-mono font-bold text-xs"
                >
                  +
                </button>
            ...
            <div className="flex items-center gap-2 pt-1 border-t border-slate-800">
              <button
                ...
                className="flex-1 py-2 px-2.5 rounded-xl text-xs font-bold border border-rose-500/30 bg-rose-500/15 text-rose-400 hover:bg-rose-500/25 flex items-center justify-center gap-1 active:scale-95"
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span>{language === 'hi' ? 'सूट हटाएं' : 'Remove Suit'}</span>
              </button>
              <button
                ...
                className="flex-1 py-2 px-2.5 rounded-xl text-xs font-extrabold bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-500/20 flex items-center justify-center gap-1 active:scale-95 border border-blue-400/40"
              >
                <Check className="w-3.5 h-3.5 stroke-[2.5]" />
                <span>{language === 'hi' ? 'पूर्ण करें ✓' : 'Done ✓'}</span>
              </button>
            </div>
          </div>
        </div>
